    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.cors(cors -> cors.configurationSource(corsConfigurationSource()))
            ...
            .authorizeHttpRequests(auth -> auth
                // Endpoint pubblici di benvenuto
                .requestMatchers("/", "/health", "/api-docs").permitAll()
                // Endpoint di autenticazione - SEMPRE PUBBLICI
                .requestMatchers("/api/auth/registrazione").permitAll()
                .requestMatchers("/api/auth/registrazione-con-ruolo").permitAll()
                .requestMatchers("/api/auth/login").permitAll()
                .requestMatchers("/api/auth/**").permitAll()
                // Endpoint pubblici per prodotti ed eventi
